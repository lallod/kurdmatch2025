...
const Messages = () => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'typing': return <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />;
        ...
        <div className="bg-black/20 backdrop-blur shadow-sm border-b border-white/20 sticky top-0 z-10">
          <div className="flex items-center p-4">
            ...
            <div className="relative">
              ...
              {conversation.online && (
                <span className="absolute bottom-0 right-2 h-3 w-3 rounded-full bg-green-500 border-2 border-white ring-1 ring-green-400/50"></span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              {conversation.notifications.includes('online') && (
                <Badge className="bg-green-500/20 text-green-300 border-green-500/30">
                  Online
                </Badge>
              )}
              {conversation.notifications.includes('viewed_profile') && (
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
          <div className="text-center space-y-2">
            ...
            <div className="flex justify-center gap-4 mt-4">
              ...
              {totalUnreadMessages > 0 && (
                <Badge className={`bg-gradient-to-r ${getUrgencyColor(sortedConversations.find(c => c.unread)?.lastMessageTime || new Date())} text-white`}>
          ...
          <div className="relative z-10">
            ...
            {sortedConversations.some(c => c.priority === 'high' && c.unread) && (
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {sortedConversations
                    .filter(c => c.priority === 'high' && c.unread)
                    .map((conversation) => (
                      <Card 
                        key={conversation.id} 
                        className="overflow-hidden backdrop-blur-md bg-red-500/10 border border-red-400/30 hover:bg-red-500/20 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl animate-pulse"
                        ...
                        <CardContent className="p-3">
                          <div className="flex items-center gap-3">
                            <div className="relative">
                              ...
                              {conversation.online && (
                                <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full bg-green-500 border-2 border-white ring-1 ring-green-400/50"></span>
                              )}
                            </div>
                            ...
                            <div className="flex flex-col items-end gap-1">
                              ...
                              {conversation.unreadCount > 0 && (
                                <div className="bg-gradient-to-r from-red-500 to-red-600 text-white text-xs rounded-full px-2 py-1 font-bold">
            ...
            <div className="space-y-3">
              {sortedConversations.map((conversation) => (
                <Card 
                  key={conversation.id} 
                  className={`overflow-hidden backdrop-blur-md border transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl ${
                    conversation.unread 
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between">
                          <p className={`text-sm truncate ${conversation.unread ? 'font-medium text-white' : 'text-purple-200'}`}>
                            {conversation.isTyping ? (
                              <span className="italic text-blue-300">typing...</span>
                            ) : (
                              conversation.lastMessage
                            )}
                          </p>
                          ...
                          <div className="flex items-center gap-1 ml-2">
                            {conversation.notifications.map((notification, index) => (
                              <div 
                                key={index}
                                className="text-purple-300 opacity-70"
                                title={notification.replace('_', ' ').toUpperCase()}
                              >
