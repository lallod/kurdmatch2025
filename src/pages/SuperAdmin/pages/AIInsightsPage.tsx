  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">AI Conversation Insights</h1>
          <p className="text-white/60 mt-1">View AI-generated conversation analysis ({totalCount} insights)</p>
        </div>
      ...
      <Card className="bg-[#1a1a1a] border-white/10">
        ...
        <CardContent>
          {loading ? (
            <div className="text-center py-8 text-white/60">Loading AI insights...</div>
          ) : filteredInsights.length === 0 ? (
            <div className="text-center py-8 text-white/60">No AI insights found</div>
          ) : (
            <div className="space-y-4">
              {filteredInsights.map((insight) => (
                <div
                  key={insight.id}
                  className="p-5 bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-colors"
                >
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      ...
                      {insight.conversation_summary && (
                        <div className="mb-3">
                          <p className="text-white/80 text-sm leading-relaxed">{insight.conversation_summary}</p>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        ...
                        {insight.shared_interests && Array.isArray(insight.shared_interests) && insight.shared_interests.length > 0 && (
                          <div className="p-3 bg-white/5 rounded-lg">
                            <p className="text-white/60 text-xs mb-2">Shared Interests</p>
                      ...
                      {insight.suggested_topics && Array.isArray(insight.suggested_topics) && insight.suggested_topics.length > 0 && (
                        <div className="p-3 bg-blue-500/5 rounded-lg border border-blue-500/10">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-blue-500 text-xs font-medium">Suggested Topics</p>
                          </div>
