...
export const useLandingPageContent = (retryCount = 0) => {
      ...
      try {
        ...
        if (error) {
          ...
          if (error.code === 'PGRST116') {
            ...
            toast.error(t('admin.could_not_load_landing', 'Could not load landing page content from the database.'));
          }
        } else if (data) {
          ...
          if (isValidLandingPageContent(contentData)) {
            ...
            toast.error(t('admin.invalid_content_format', 'The content stored in the database has an invalid format.'));
          }
        } else {
        ...
        toast.error(t('admin.unexpected_error_loading', 'An unexpected error occurred while loading the content.'));
      } finally {
        setLoading(false);
      }
    };
    ...
    const initializeDefaultContent = async () => {
      try {
        ...
        if (insertError) {
          ...
          toast.error(t('admin.could_not_save_default', 'Could not save default content to the database.'));
        } else {
  ...
  const updateFeatureTitle = useCallback((value: string) => {
    setContent(prev => {
      ...
      return { ...prev, features: { ...prev.features, title: value } };
    });
  }, []);
  ...
  const updateKurdistanSection = useCallback((field: keyof LandingPageContent['kurdistan'], value: string | string[]) => {
    setContent(prev => {
      ...
      return { ...prev, kurdistan: { ...prev.kurdistan, [field]: value } };
    });
  }, []);
  ...
  const updateKurdistanPoint = useCallback((section: 'leftPoints' | 'rightPoints', index: number, value: string) => {
    setContent(prev => {
      ...
      newPoints[index] = value;
      return { ...prev, kurdistan: { ...prev.kurdistan, [section]: newPoints } };
    });
  }, []);
  ...
  const saveChanges = useCallback(async () => {
    if (!content) {
      toast.error(t('admin.no_content_to_save', 'No content to save. Please try reloading the page.'));
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        toast.error(t('admin.could_not_save_landing', 'Could not save landing page content to the database.'));
      } else {
        console.log('Content saved successfully');
        toast.success(t('admin.landing_saved', 'Landing page content has been updated successfully.'));
      }
    } catch (error: any) {
      console.error('Error:', error);
      setError(`An unexpected error occurred: ${error.message}`);
      toast.error(t('admin.unexpected_error_saving', 'An unexpected error occurred while saving the content.'));
    } finally {
      setSaving(false);
    }
  }, [content, t]);
