  ...
  const loadData = async () => {
    ...
    try {
      const [profileRes, detailsRes, prefsRes] = await Promise.all([
        supabase.from('profiles').select('id, name, age, gender, location, profile_image, bio, verified, video_verified, occupation, interests, hobbies, values, languages, height, body_type, ethnicity, religion, kurdistan_region, education, relationship_goals, zodiac_sign, personality_type, company, work_environment, career_ambitions, exercise_habits, dietary_preferences, smoking, drinking, sleep_schedule, have_pets, want_children, love_language, communication_style, ideal_date, family_closeness, creative_pursuits, weekend_activities, political_views, work_life_balance, travel_frequency, last_active, created_at, transportation_preference, children_status, phone_number, phone_verified, latitude, longitude, geo_location, notification_preferences').eq('id', userId).maybeSingle(),
