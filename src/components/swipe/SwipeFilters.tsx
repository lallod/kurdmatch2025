...
export const SwipeFilterSidebar: React.FC<SwipeFilterSidebarProps> = ({ onApplyFilters, currentFilters, open, onOpenChange }) => {
  ...
  const handleApply = () => {
    if (!isSubscribed) { toast.error('Premium feature', { description: 'Subscribe to use advanced filters', icon: '⭐' }); return; }
  ...
  const handleReset = () => { setAgeMin('18'); setAgeMax('99'); setLocation(''); setReligion(''); onApplyFilters({}); onOpenChange(false); toast.success('Filters reset'); };

  if (!open) return null;

  return (
    <div className="fixed inset-0 z-50 bg-black/50" onClick={() => onOpenChange(false)}>
      <div className="fixed top-0 right-0 h-full w-[90vw] sm:w-[85vw] max-w-sm bg-background shadow-2xl transform transition-transform duration-300" onClick={(e) => e.stopPropagation()}>
        <Sidebar className="w-full h-full border-0">
          <SidebarHeader className="border-b border-border/20 p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <SlidersHorizontal className="w-4 h-4 sm:w-5 sm:h-5 text-foreground" />
                <h2 className="text-base sm:text-lg font-semibold text-foreground">Swipe Filters</h2>
                {!isSubscribed && (<span className="text-[10px] sm:text-xs bg-gradient-to-r from-yellow-400 to-yellow-600 text-black px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full font-medium">Premium</span>)}
              </div>
          ...
          <SidebarContent className="p-3 sm:p-4">
            {!isSubscribed && (
              <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-2 sm:gap-3">
                  ...
                  <div><p className="text-xs sm:text-sm font-semibold text-yellow-100">Premium Feature</p><p className="text-[10px] sm:text-xs text-yellow-200/80 mt-1">Subscribe to unlock advanced filters</p></div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                <Input type="text" placeholder="City or region..." value={location} onChange={(e) => setLocation(e.target.value)} className="bg-muted/50 border-border text-foreground placeholder:text-muted-foreground h-9 text-sm" disabled={!isSubscribed} />
              </div>
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                <Select value={religion} onValueChange={setReligion} disabled={!isSubscribed}>
                  <SelectTrigger className="bg-muted/50 border-border text-foreground h-9"><SelectValue placeholder="Select religion..." /></SelectTrigger>
                  <SelectContent className="bg-card border-border max-h-[200px]">
                    ...
                    <SelectItem value="Atheist">Atheist</SelectItem><SelectItem value="Other">Other</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <SidebarFooter className="border-t border-border/20 p-3 sm:p-4">
            <div className="flex gap-2">
              <Button onClick={handleReset} variant="outline" className="flex-1 bg-muted/50 border-border text-foreground hover:bg-muted h-9 text-sm">Reset</Button>
              <Button onClick={handleApply} className="flex-1 bg-gradient-to-r from-primary to-pink-500 hover:from-primary/90 hover:to-pink-600 h-9 text-sm" disabled={!isSubscribed}>Apply Filters</Button>
            </div>
          </SidebarFooter>
        </Sidebar>
      </div>
