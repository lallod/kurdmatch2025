      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent backdrop-blur-sm p-2 sm:p-3">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-1 mb-1">
              {profile.occupation && <Badge variant="secondary" className="text-xs bg-black/40 backdrop-blur-sm text-white border-white/20 px-1.5 py-0.5">
                  {profile.occupation}
                </Badge>}
        ...
        <div className="mt-4 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-3 sm:p-4 shadow-2xl animate-fade-in">
          ...
          {isGenerating ? <div className="flex items-center gap-2 text-white/80 text-xs sm:text-sm">
              ...
              {/* Interest Tags */}
              {profile.interests && profile.interests.length > 0 && <div className="flex flex-wrap gap-1">
                  {profile.interests.slice(0, 3).map((interest, index) => <Badge key={index} className="text-xs bg-gradient-to-r from-purple-500/20 to-pink-500/20 backdrop-blur-sm text-purple-200 border-purple-400/30 px-1.5 py-0.5">
                      {interest}
                    </Badge>)}
                  {profile.interests.length > 3 && <Badge className="text-xs bg-white/10 backdrop-blur-sm text-white/70 border-white/20 px-1.5 py-0.5">
