    ...
    <div className="flex justify-between mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/20">
      {currentStep > 1 ? (
        ...
        >
          ...
          <span className="hidden xs:inline">{t('auth.back', 'Back')}</span>
        </Button>
      ) : (
        <div></div>
      )}
        ...
        >
          {isSubmitting ? (
            <>
              ...
              <span className="hidden xs:inline">{t('auth.creating_account', 'Creating Account...')}</span>
              <span className="xs:hidden">{t('auth.creating', 'Creating...')}</span>
            </>
          ) : (
            <>
              <span className="hidden xs:inline">{t('auth.complete_registration', 'Complete Registration')}</span>
              <span className="xs:hidden">{t('auth.complete', 'Complete')}</span>
            </>
          )}
        </Button>
      )}
