...
### 13.1 Frontend
- **ErrorBoundary**: React error boundary wrapper rundt kritiske komponenter (Swipe, Messages, Profile)
- **Toast**: Sonner brukes for alle brukervenlige feilmeldinger
- **Loading states**: `LoadingState`-komponent med spinner
- **Empty states**: `EmptyState`-komponent (underutnyttet)
...
### 13.3 Logging
- `src/utils/logger.ts` - produksjonslogger (delvis implementert)
...
4. Paginering mangler på flere endepunkter
5. Ghost user generator fortsatt i klient-bundle

### 14.2 Foreslåtte forbedringer
1. **TURN-servere**: Legg til Twilio/Metered TURN for pålitelige videoanrop
2. **Server-side paginering**: Implementer cursor-basert paginering for meldinger og matcher
...
6. **CDN-optimalisering**: Bildekomprimering og lazy loading
7. **Overvåking**: Sentry eller tilsvarende for feilrapportering
8. **A/B-testing**: Koble ab_tests-tabellen til faktisk frontend-logikk
9. **Email-kampanjer**: Implementer SendGrid/Resend for kampanjefunksjonalitet
...
- Indekser bør verifiseres for `ilike`-søk og geo-spørringer
